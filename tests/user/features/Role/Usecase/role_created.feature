Feature: 創建角色
    ...
    Scenario Outline: 創建角色成功
        ...
        Examples:
            ...
            |admin|this is admin role|read:all,write:all,delete:all,update:all|
            |develop|this is develop role|read:all|
            |owner|this is owner role|read:all,write:all,update:all|
            |test|this is test role|read:all,delete:all,update:all|

    @TestCaseKey=LA-T11
    Scenario Outline: 創建一個已存在的角色
        
        Given 提供 <name>, <description>, <permissions>
        When 嘗試創建一個已存在的角色
        Then 應該返回一個錯誤，說明角色已存在
    ...
    Scenario Outline: 提供的權限格式不正確
        ...
        Examples:
            |name|description|permissions|
            |invalid|this is invalid role|read:all,write:all,delete:all,update:all,invalid:all|
    ...
    Scenario Outline: 提供的名稱格式不正確
        ...
        Examples:
            |name|description|permissions|
            |@|this is invalid role|read:all,write:all,delete:all,update:all|
    ...
    @TestCaseKey=LA-T16
    Scenario Outline: 提供的角色名稱或描述長度超過最大值
